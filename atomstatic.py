...
def GuessMaxx(h, sizeX, sizeY, sigma1, sigma2):
    ...
    # create a blank convolution kernel 
    ...
    for i in range(sizeX):
        for j in range(sizeY):
            core[i][j] = math.e**(-((midx-j+1)**2/sigma2**2 +
                                    (midy-i+1)**2/sigma1**2))/(2*math.pi*sigma1*sigma2)
            sum += core[i][j]
    ...
    for i in range(sizeX):
        for j in range(sizeY):
            core[i][j] -= average*0.5

    # assign value to the convolution kernel

    print(core)
    print(sum, average)
    f = cv2.filter2D(h, -1, core)*2
    #process convolution operation get f
    ...
    for i in range(a):
        for j in range(b):
            ...
            if formf[i][j] < 30:
                formf[i][j] = 0
    # normalize the brightness of f and remove pixel not bright enough
...
for i in range(1, num):
    if atomnum[i] < 6 or atomsum[i] < 350:
        continue
...
for i in range(1, num):
    ...
    atomp[axisx[i]][axisy[i]] = searchnear(axisx[i], axisy[i], 40)
'''    print (numberofpoint)
    deltar=0
    tx=ty=0
    if numberofpoint==0:
...
'''
atomlin = cv2.GaussianBlur(atomp, (101, 101), 1)
